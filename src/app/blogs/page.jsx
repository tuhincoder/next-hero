...
const BlogsPage = () => {
    ...
    return (
        <div className="p-5 ">
            {blogs.map(blog => <div className="border border-gray-700  p-5 space-y-2 " key={blog.id}>
                <h2 className="text-2xl font-serif">{blog.title}</h2>
                <p>{blog.description}</p>
                <button className="btn bg-slate-600 p-3">
                    <Link href={`/blogs/${blog.id}`}>Description</Link>
                </button>
            </div>)}
        </div>
